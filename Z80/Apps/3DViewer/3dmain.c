...
byte file_read_line()
{
	...
	do
	{
		...
		do
		{
			c = io_read(163); // read byte from file
			filesize--;
		}
		while (filesize > 0 && (c == 0x0a || c == 0x0d || c == 0x20));

		// read next line
		i = 0;
		p = line_buf;
		while (filesize > 0 && c != 0x0a && c != 0x0d)
		{
...
byte load_model(char *file_name)
{
	...
	if (!fopen(file_name, FM_READ, &filesize)) {
		puts("File not found!");
		return 0;
	}
